...
void ObjectListener::openFile(bool newfile)
{
    ...
    //connect(table,SIGNAL(clicked(QModelIndex)),this,
    //        SLOT(tableView_clicked(QModelIndex)));
    //connect(table,SIGNAL(viewportEntered()),this,SLOT(tableView_viewportEntered()));
...
void ObjectListener::menuClicked(QAction *action)
{
    ...
    }else if(action==wnd->actionAbout)
    {
        ...
        QGraphicsOpacityEffect* fade_effect = new QGraphicsOpacityEffect(but);
        but->setGraphicsEffect(fade_effect);
...
void ObjectListener::TabView_mousePressed(QMouseEvent *event)
{
    ...
    if(event->button()==Qt::RightButton)
    {
        ...
        QRect area=wnd->menuTableView->geometry();
        area.setLeft(p.x());
        area.setTop(p.y());
        wnd->menuTableView->setGeometry(area);
        QAction *result=wnd->menuTableView->exec();
        if(result==wnd->actionEdit)
        {
            tableDoubleClicked(wnd->tables[index]->currentIndex());
        }else if(result==wnd->actionRemove)
        {
            deleteItem(index);
        }
    }
