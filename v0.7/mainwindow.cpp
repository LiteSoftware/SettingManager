    ...
    ui(new Ui::MainWindow)
{
    ...
    pMenuEdit=QSharedPointer<QMenu>
            (new QMenu("Edit",ui->menuBar));

    pActionEdit=QSharedPointer<QAction>
            (new QAction(QIcon(":pictures/forms/edit"),
                                                    QObject::tr("Edit"),this));
    pActionEdit->setShortcut(QKeySequence("Ctrl+e"));

    pActionRemove=QSharedPointer<QAction>
            (new QAction(QIcon(":pictures/forms/delete"),QObject::tr("Remove"),this));
    pActionRemove->setShortcut(QKeySequence("Ctrl+r"));

    commands.append(pActionEdit.data());
    commands.append(pActionRemove.data());
    pMenuEdit->addActions(commands);

    ui->menuBar->addMenu(pMenuEdit.data());
    //----------------menu menuHelp----------------
    ...
    connect(pMenuEdit.data(),SIGNAL(triggered(QAction*)),listener,SLOT(menuClicked(QAction*)));
...
void MainWindow::resizeEvent(QResizeEvent *event)
{
    ...
    pBtnNew->setGeometry(4,menurect.height()+2, ui->line->geometry().left()-5,35);
